...
src/
  ...
  data/stargateData.ts 96 entrées GÉNÉRÉES depuis des tuples (l'index = l'ordre)
...
## État : tout fonctionne en mock (aucun backend)

`npm run dev` → http://localhost:5173. Vérifier avec `npm run build` (tsc strict).

## Feuille de route (ordre suggéré)

1. **TMDb** : remplir synopsis/notes/images des 90+ épisodes Stargate (fetchContentDetails)
